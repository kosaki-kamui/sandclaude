...
async def save_policy_endpoint(
    name: str, config: dict, auth: AuthResult = Depends(_require_auth)
) -> dict:
...
@router.get("/policies")
async def list_policies_endpoint(auth: AuthResult = Depends(_require_auth)) -> list[dict]:
    require_scope(auth, "admin:policies")
...
async def get_policy_endpoint(name: str, auth: AuthResult = Depends(_require_auth)) -> dict:
    require_scope(auth, "admin:policies")
...
async def delete_policy_endpoint(name: str, auth: AuthResult = Depends(_require_auth)) -> dict:
    require_scope(auth, "admin:policies")
